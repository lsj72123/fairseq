...
def _chunk_iterator(itr, chunk_size):
    ...
    for x in itr:
        chunk.append(x)
        if len(chunk) == chunk_size:  # here only when len(chunk) = chunk_size before output
            yield chunk
            chunk = []
    if len(chunk) > 0:
        yield chunk
...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def __init__(
        ...
        self._frozen_batches = (  # stored as tuple
    ...
    def next_epoch_itr(
        ...
        self.epoch = self.next_epoch_idx    # update self.epoch
    ...
    def _get_iterator_for_epoch(
        ...
        if self._supports_prefetch:  # todo
