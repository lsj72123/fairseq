...
def gen_parser_from_dataclass(
    ...
    for k in dataclass_instance._get_all_attributes():  # all the args
        field_name = argparse_name(dataclass_instance._get_name(k))
        field_type = dataclass_instance._get_type(k)
        if field_name is None:  # private name
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, FairseqDataclass):
            gen_parser_from_dataclass(parser, field_type(), delete_default) # iteration
            continue
...
def _override_attr(
        sub_node: str,
        data_class: Type[FairseqDataclass],
        args: Namespace
) -> List[str]:
    overrides = []
