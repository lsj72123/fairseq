...
def get_training_parser(default_task="translation"):
    parser = get_parser("Trainer", default_task)  # common config
...
def get_parser(desc, default_task="translation"):
    ...
    gen_parser_from_dataclass(parser, CommonConfig())  # add Config into parse.

    from fairseq.registry import REGISTRIES

    for registry_name, REGISTRY in REGISTRIES.items():  # todo: what is the REGISTERS in here
        parser.add_argument(
    ...
    from fairseq.tasks import TASK_REGISTRY  # all the registered task
...
def add_preprocess_args(parser):
    ...
    # fmt: off
    group.add_argument("-s", "--source-lang", default=None,
                       metavar="SRC",
                       help="source language")
    group.add_argument("-t", "--target-lang", default=None,
                       metavar="TARGET",
                       help="target language")
    group.add_argument("--trainpref", default=None,
                       metavar="FP",
                       help="train file prefix (also used to build dictionaries)")
    group.add_argument("--validpref", default=None,
                       metavar="FP",
                       help="comma separated, valid file prefixes "
                            "(words missing from train set are replaced with <unk>)")
    group.add_argument("--testpref", default=None,
                       metavar="FP",
                       help="comma separated, test file prefixes "
                            "(words missing from train set are replaced with <unk>)")
    group.add_argument("--align-suffix", default=None,
                       metavar="FP",
                       help="alignment file suffix")
    group.add_argument("--destdir", default="bin",
                       metavar="DIR",
                       help="destination dir")
    group.add_argument("--thresholdtgt", default=0, type=int,
                       metavar="N",
                       help="map words appearing less than threshold times to unknown")
    group.add_argument("--thresholdsrc", default=0, type=int,
                       metavar="N",
                       help="map words appearing less than threshold times to unknown")
    ...
    group.add_argument("--nwordstgt", default=-1, type=int,
                       metavar="N",
                       help="number of target words to retain")
    group.add_argument("--nwordssrc", default=-1, type=int,
                       metavar="N",
                       help="number of source words to retain")
    group.add_argument("--alignfile", default=None,
                       metavar="ALIGN",
                       help="an alignment file (optional)")
    parser.add_argument('--dataset-impl', default='mmap', choices=get_available_dataset_impl(),
                        metavar='FORMAT',
...
def parse_args_and_arch(
    ...
    # Post-process args.
    if (hasattr(args, "batch_size_valid") and args.batch_size_valid is None) or \
            not hasattr(args, "batch_size_valid"):
