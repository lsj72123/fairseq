...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if self.cfg.distributed_training.ddp_backend == "fully_sharded":    # todo
        ...
        if self.quantizer is not None:  # todo
            self.quantizer.set_trainer(self)

        # get detailed cuda environment
        if self.cuda:
            self.cuda_env = utils.CudaEnvironment()
            if self.data_parallel_world_size > 1:   # todo
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:    # todo
        ...
        if extra_state is not None:  # todo
    ...
    def lr_scheduler(self):
        if self._lr_scheduler is None:  # here lr_scheduler initialized with Fixed_scheduler
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                ...
                sample_size += sample_size_i  # sample_size contains all the tokens number
    ...
    def _check_grad_norms(self, grad_norm):
        """Check that grad norms are consistent across workers."""
        if self._grad_norm_buf is not None:  # todo
